...
const TradeHistory = ({ trades, onDelete, onEdit, currencySymbol, colors }) => {
    ...
    const handleTogglePair = (pair) => {
        if (pair === 'ALL') {
            ...
            if (newSelection.length === 0) setFilterPairs(['ALL']);
            else setFilterPairs(newSelection);
        }
    };
    ...
    const handleToggleTag = (tag) => {
        if (tag === 'ALL') {
            ...
            if (newSelection.length === 0) setFilterTags(['ALL']);
            else setFilterTags(newSelection);
        }
    };
    ...
    const enrichedTrades = useMemo(() => {
        ...
        return chronological.map(trade => {
            ...
            // (Logique calcul optionnelle pour affichage direct)

            return {
                ...trade,
                percentString: trade.pair !== 'SOLDE' ? percent.toFixed(2) + '%' : '-',
                        ...
                        <div key={trade.id} onClick={() => setSummaryTrade(trade)} className="relative bg-white/60 dark:bg-neutral-900/60 backdrop-blur-xl p-4 rounded-2xl border border-white/20 dark:border-white/5 shadow-sm active:scale-[0.99] transition-transform">
                            ...
                            <div className="flex justify-between items-start mb-3">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div className="font-black text-gray-900 dark:text-white text-base flex items-center gap-2">
                                            {isTransfer ? 'Mouvement' : trade.pair}
                                            {!isTransfer && score !== undefined && <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full flex items-center gap-0.5 border ${scoreColor.replace('text-', 'border-').replace('bg-', 'bg-opacity-10 ')}`}><BrainCircuit size={8} /> {score}%</span>}
                                        </div>
                            ...
                            <div className="flex justify-between items-center pt-3 border-t border-gray-100 dark:border-neutral-800/50">
                                <div className="flex gap-2 items-center">
                                    <span className="inline-flex items-center justify-center gap-1 px-2.5 py-1 rounded-lg text-[10px] font-bold border" style={{ color: typeColor, backgroundColor: getBgColor(typeColor), borderColor: getBorderColor(typeColor) }}>
                                         {isTransfer ? (trade.type === 'DEPOSIT' ? 'DÉPÔT' : 'RETRAIT') : <>{trade.type === 'BUY' ? <ArrowUpCircle size={10}/> : <ArrowDownCircle size={10}/>} {trade.type}</>}
                                     </span>
                                    {!isTransfer && <span className="text-[11px] font-medium text-gray-400 dark:text-gray-500 flex items-center gap-1"><Hash size={10}/> {trade.lot}</span>}
                                </div>
                                <div className="flex items-center gap-1">
                                    <button onClick={() => setSummaryTrade(trade)} className="p-2 rounded-full bg-gray-50 dark:bg-neutral-800 text-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/20"><Eye size={16} /></button>
                                    <button onClick={() => onEdit(trade)} className="p-2 rounded-full bg-gray-50 dark:bg-neutral-800 text-gray-500 dark:text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/20"><Pencil size={16} /></button>
                                    <button onClick={() => onDelete(trade.id)} className="p-2 rounded-full bg-gray-50 dark:bg-neutral-800 text-gray-500 dark:text-gray-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/20"><Trash2 size={16} /></button>
                                </div>
                            </div>
                        </div>
                    );
