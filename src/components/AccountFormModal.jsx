...
import { X, Save, Briefcase, DollarSign, Palette, ChevronDown, Check, Euro, PoundSterling } from 'lucide-react';
...
const AccountFormModal = ({ isOpen, onClose, onSave, accountToEdit }) => {
    ...
    // Click outside pour fermer la devise
    useEffect(() => {
                ...
                <div className="overflow-y-auto p-6 space-y-6 scrollbar-hide">
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <input type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className={inputClass} placeholder="Ex: Challenge FTMO" autoFocus />
                        </div>
                            ...
                            <div>
                                ...
                                <div className="h-[48px] w-full relative rounded-xl overflow-hidden border border-gray-100 dark:border-neutral-800 cursor-pointer bg-white dark:bg-neutral-950 flex items-center px-2">
                                    ...
                                    <div className="flex items-center gap-3 w-full pl-2">
                                        ...
                                        ></div>
                                        <span className="text-xs font-mono text-gray-500 uppercase truncate">{formData.color}</span>
                                    </div>
                                </div>
                            </div>

                            {/* SÉLECTEUR DE DEVISE (STYLE SETTINGS) */}
                            <div ref={currencyDropdownRef} className="relative">
                                ...
                                <button type="button" onClick={() => setIsCurrencyOpen(!isCurrencyOpen)} className={`${inputClass} flex items-center justify-between text-left`}>
                                    <div className="flex items-center gap-2">
                                        <div className="w-6 h-6 rounded-full bg-gray-100 dark:bg-neutral-800 flex items-center justify-center text-gray-600 dark:text-gray-300">
                                        ...
                                        {CURRENCIES.map((curr) => {
                                            ...
                                            return (
                                                <button
                                                    key={curr.code}
                                                    type="button"
                                                    onClick={() => { setFormData(prev => ({ ...prev, currency: curr.code })); setIsCurrencyOpen(false); }}
                                                ...
                                                >
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <span className="font-bold text-gray-700 dark:text-gray-300 text-sm">{curr.code}</span>
                                                    </div>
                    ...
                    <div className="space-y-4 pt-4 border-t border-gray-100 dark:border-neutral-800">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" value={formData.broker} onChange={e => setFormData({...formData, broker: e.target.value})} className={inputClass} placeholder="Ex: Vantage" />
                            </div>
                            <div>
                                <label className={labelClass}>Plateforme</label>
                                <input type="text" value={formData.platform} onChange={e => setFormData({...formData, platform: e.target.value})} className={inputClass} placeholder="Ex: VTMarkets" />
                            </div>
                        </div>
                        <div>
                            <label className={labelClass}>Description / Notes</label>
                            <textarea value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} className={inputClass + " min-h-[80px]"} placeholder="Détails du compte..." />
                        </div>
                    </div>

                    {/* 3. Règles de Commission */}
                    <div className="space-y-4 pt-4 border-t border-gray-100 dark:border-neutral-800">
                        <h3 className="text-sm font-bold text-indigo-500 flex items-center gap-2"><DollarSign size={16}/> Règles de Commission</h3>
                        <div className="grid grid-cols-3 gap-3">
                            <div>
                                <label className={labelClass}>% par lot</label>
                                <div className="relative">
                                    <input type="number" step="0.01" value={formData.commission_pct} onChange={e => setFormData({...formData, commission_pct: parseFloat(e.target.value)})} className={inputClass + " pl-8"} placeholder="0" />
                                    <span className="absolute left-3 top-3.5 text-gray-400 font-bold text-xs">%</span>
                                </div>
                            </div>
                            <div>
                                <label className={labelClass}>Min ($)</label>
                                <input type="number" step="0.01" value={formData.commission_min} onChange={e => setFormData({...formData, commission_min: parseFloat(e.target.value)})} className={inputClass} placeholder="Min" />
                            </div>
                            <div>
                                <label className={labelClass}>Max ($)</label>
                                <input type="number" step="0.01" value={formData.commission_max} onChange={e => setFormData({...formData, commission_max: parseFloat(e.target.value)})} className={inputClass} placeholder="Max" />
                            </div>
                        </div>
                        <p className="text-[10px] text-gray-400 italic">Ces règles seront utilisées pour calculer automatiquement les frais sur vos futurs trades.</p>
                    </div>
