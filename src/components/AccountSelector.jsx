...
const AccountSelector = ({ accounts, currentAccount, onSelect, onOpenCreate, onOpenEdit, onDelete }) => {
    ...
    return (
        <div className="mb-6">
            ...
            {/* --- VERSION MOBILE : DROPDOWN CUSTOM --- */}
                    ...
                    <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-[#1a1a1a] border border-gray-100 dark:border-neutral-800 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 origin-top">
                        <div className="max-h-[60vh] overflow-y-auto scrollbar-hide p-2 space-y-1">
                            {accounts.map(acc => (
                                <div
                                    key={acc.id}
                                    onClick={() => { onSelect(acc); setIsOpen(false); }}
                                    className={`flex items-center justify-between p-3 rounded-xl transition-all cursor-pointer ${currentAccount.id === acc.id
                        ...
                        {/* Footer Actions (Créer / Configurer) */}
                        <div className="p-2 border-t border-gray-100 dark:border-neutral-800 bg-gray-50/50 dark:bg-neutral-900/50 grid grid-cols-2 gap-2">
                            <button
                                onClick={() => { onOpenEdit(currentAccount); setIsOpen(false); }}
                                className="flex items-center justify-center gap-2 p-3 rounded-xl text-xs font-bold text-gray-600 dark:text-gray-400 hover:bg-white dark:hover:bg-neutral-800 border border-transparent hover:border-gray-200 dark:hover:border-neutral-700 transition-all"
                            >
                                <Settings size={14} /> Configurer
                            </button>
                            <button
                                onClick={() => { onOpenCreate(); setIsOpen(false); }}
                                className="flex items-center justify-center gap-2 p-3 rounded-xl text-xs font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-100 dark:border-indigo-500/30 hover:bg-indigo-100 dark:hover:bg-indigo-500/20 transition-all"
                            >
            ...
            {/* --- VERSION DESKTOP : CARTES HORIZONTALES (INCHANGÉE) --- */}
