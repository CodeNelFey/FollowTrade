...
const Home = ({ onNavigateToAuth }) => {
    return (
        <div className="relative min-h-screen w-full overflow-x-hidden text-white font-sans selection:bg-indigo-500/30 bg-black">
            ...
            {/* --- NAVBAR CORRIGÉE (SAFE AREA) --- */}
            {/* Ajout de pt-[env(safe-area-inset-top)] pour éviter l'encoche */}
            <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 pb-4 pt-[calc(1rem+env(safe-area-inset-top))] md:px-12 md:py-6 bg-black/50 backdrop-blur-xl border-b border-white/5 transition-all">
            ...
            {/* --- HERO SECTION --- */}
            {/* Ajustement du padding-top pour compenser la navbar plus grande */}
            <header className="relative pt-36 pb-12 md:pt-48 md:pb-32 px-6 flex flex-col items-center text-center max-w-5xl mx-auto z-10">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto px-4 animate-in fade-in slide-in-from-bottom-8 duration-700 delay-300">
                    <button onClick={() => onNavigateToAuth(true)} className="w-full sm:w-auto px-8 py-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl font-bold text-lg shadow-xl shadow-indigo-500/20 transition-all hover:scale-105 active:scale-95 flex items-center justify-center gap-2">
                        Commencer Gratuitement <ChevronRight size={20} />
                    </button>
                    <button onClick={() => onNavigateToAuth(false)} className="w-full sm:w-auto px-8 py-4 bg-white/5 hover:bg-white/10 text-white border border-white/10 rounded-2xl font-bold text-lg backdrop-blur-md transition-all active:scale-95">
                        Se Connecter
                    </button>
                </div>
            </header>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
                    <div className="md:col-span-2 bg-neutral-900/50 border border-white/5 rounded-3xl p-6 md:p-10 relative overflow-hidden group hover:border-white/10 transition-colors">
                        ...
                        <div className="relative z-10">
                            <div className="w-12 h-12 bg-neutral-800 rounded-xl flex items-center justify-center mb-6 text-indigo-400"><BarChart3 size={24} /></div>
                            <h3 className="text-2xl font-bold mb-3">Analyses Détaillées</h3>
                            <p className="text-gray-400 leading-relaxed text-sm md:text-base">Ne tradez plus à l'aveugle. Obtenez des graphiques précis sur votre Winrate, votre Risk/Reward et l'évolution de votre capital.</p>
                        </div>
                    </div>
                    <div className="bg-neutral-900/50 border border-white/5 rounded-3xl p-6 md:p-10 relative overflow-hidden group hover:border-white/10 transition-colors">
                        ...
                        <div className="relative z-10">
                            <div className="w-12 h-12 bg-neutral-800 rounded-xl flex items-center justify-center mb-6 text-emerald-400"><Shield size={24} /></div>
                            <h3 className="text-xl font-bold mb-3">Privé & Sécurisé</h3>
                            <p className="text-gray-400 text-sm">Vos données sont personnelles. Nous ne les partageons avec personne.</p>
                        </div>
                    </div>
                    <div className="bg-neutral-900/50 border border-white/5 rounded-3xl p-6 md:p-10 relative overflow-hidden group hover:border-white/10 transition-colors">
                        <div className="absolute top-0 right-0 w-40 h-40 bg-amber-600/10 rounded-full blur-[80px] pointer-events-none"></div>
                        <div className="relative z-10">
                            <div className="w-12 h-12 bg-neutral-800 rounded-xl flex items-center justify-center mb-6 text-amber-400"><Zap size={24} /></div>
                            <h3 className="text-xl font-bold mb-3">Saisie Rapide</h3>
                            <p className="text-gray-400 text-sm">Ajoutez vos trades en quelques secondes. Importez vos captures d'écran via OCR.</p>
                        </div>
                    </div>
                    <div className="md:col-span-2 bg-neutral-900/50 border border-white/5 rounded-3xl p-6 md:p-10 relative overflow-hidden group hover:border-white/10 transition-colors">
                        ...
                        <div className="relative z-10">
                            <div className="w-12 h-12 bg-neutral-800 rounded-xl flex items-center justify-center mb-6 text-purple-400"><Smartphone size={24} /></div>
                            <h3 className="text-2xl font-bold mb-3">100% Mobile Friendly</h3>
                            <p className="text-gray-400 leading-relaxed text-sm md:text-base">Accédez à votre journal partout. Que vous soyez sur votre bureau ou en déplacement, l'interface s'adapte parfaitement.</p>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section className="px-6 py-20 text-center">
                <div className="max-w-3xl mx-auto bg-gradient-to-br from-indigo-900/50 to-purple-900/50 border border-white/10 rounded-3xl p-8 md:p-16 relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        ...
                        <p className="text-indigo-200 mb-8 max-w-lg mx-auto">Rejoignez la communauté et transformez votre trading dès aujourd'hui.</p>
                        <button onClick={() => onNavigateToAuth(true)} className="w-full sm:w-auto bg-white text-indigo-900 hover:bg-gray-100 px-8 py-4 rounded-xl font-bold text-lg transition-transform hover:scale-105 active:scale-95 shadow-xl">Créer mon compte gratuit</button>
                    </div>
                </div>
            </section>

            <footer className="border-t border-white/5 py-12 text-center text-gray-500 text-sm">
                <div className="flex items-center justify-center gap-2 mb-4 text-gray-400"><TrendingUp size={16} /><span className="font-bold text-gray-300">FollowTrade</span></div><p>&copy; 2025 Tous droits réservés.</p>
            </footer>
        </div>
    );
};
