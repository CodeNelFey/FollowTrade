        ...
        <div className={`h-[100dvh] w-full transition-colors duration-300 ${isDark ? 'dark' : ''} overflow-hidden flex flex-col relative`} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}>
            ...
            <div className="relative z-10 flex flex-1 h-full overflow-hidden">
                ...
                <div className="flex-1 flex flex-col h-full overflow-hidden relative">
                            ...
                            {activeTab === 'journal' && (
                                <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-6">
                                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 mb-8">
                                        <div className="col-span-2 lg:col-span-1 rounded-3xl p-4 md:p-6 text-white relative overflow-hidden shadow-lg transition-all" style={{ background: `linear-gradient(135deg, ${activeColor}, #000000)` }}>
                                            <div className="absolute top-0 right-0 w-24 h-24 bg-white/10 rounded-full -mr-8 -mt-8 blur-xl"></div>
                                            <div className="relative z-10"><div className="flex items-center gap-2 text-white/80 mb-1 text-[10px] md:text-xs font-bold uppercase tracking-wider"><Wallet size={16} /> Solde Actuel</div><div className="text-2xl md:text-4xl font-black tracking-tight">{currentBalance.toLocaleString('en-US', { style: 'currency', currency: currencyCode })}</div><div className="mt-1 text-[10px] md:text-xs opacity-60 font-medium truncate">{currentAccount?.name || 'Sélectionnez un compte'}</div></div>
                                        </div>
                                        <div className="bg-white dark:bg-neutral-900 rounded-3xl p-3 md:p-5 border border-gray-100 dark:border-neutral-800 shadow-sm flex flex-col justify-center">
                                            <div className="flex items-center gap-2 text-gray-400 mb-1 text-[10px] md:text-xs font-bold uppercase tracking-wider"><BrainCircuit size={16} className={avgDiscipline >= 90 ? 'text-emerald-500' : (avgDiscipline >= 50 ? 'text-indigo-500' : 'text-rose-500')} /><span>Discipline</span></div><div className={`text-xl md:text-3xl font-black ${avgDiscipline >= 90 ? 'text-emerald-500' : (avgDiscipline >= 50 ? 'text-gray-800 dark:text-white' : 'text-rose-500')}`}>{avgDiscipline}%</div><div className="text-[10px] text-gray-400 font-medium">Moyenne globale</div>
                                        </div>
                                        <div className="bg-white dark:bg-neutral-900 rounded-3xl p-3 md:p-5 border border-gray-100 dark:border-neutral-800 shadow-sm flex flex-col justify-center">
                                            <div className="flex items-center gap-2 text-gray-400 mb-1 text-[10px] md:text-xs font-bold uppercase tracking-wider"><Activity size={16} className={totalPnL >= 0 ? 'text-emerald-500' : 'text-rose-500'} /><span>Gain Total</span></div><div className={`text-xl md:text-3xl font-black ${totalPnL >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}>{totalPnL > 0 ? '+' : ''}{totalPnL.toFixed(0)}<span className="text-sm font-normal text-gray-400 ml-1">{currencySymbol}</span></div><div className="text-[10px] text-gray-400 font-medium">P&L Cumulé</div>
                                        </div>
                                        <div className="bg-white dark:bg-neutral-900 rounded-3xl p-3 md:p-5 border border-gray-100 dark:border-neutral-800 shadow-sm flex flex-col justify-center">
                                            <div className="flex items-center gap-2 text-gray-400 mb-1 text-[10px] md:text-xs font-bold uppercase tracking-wider"><TrendingUp size={16} className={totalGainPct >= 0 ? 'text-emerald-500' : 'text-rose-500'} /><span>Croissance</span></div><div className={`text-xl md:text-3xl font-black ${totalGainPct >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}>{totalGainPct > 0 ? '+' : ''}{totalGainPct.toFixed(2)}%</div><div className="text-[10px] text-gray-400 font-medium">Sur capital investi</div>
                                        </div>
                                    </div>
                    ...
                    {/* --- MENU MOBILE FIXE STYLISÉ --- */}
                    <div className="fixed bottom-0 left-0 right-0 z-40 md:hidden bg-white/80 dark:bg-black/80 backdrop-blur-xl border-t border-gray-200 dark:border-neutral-800 pb-[env(safe-area-inset-bottom)] shadow-lg">
